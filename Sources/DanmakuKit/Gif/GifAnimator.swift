...
public class GifAnimator {
  ...
  public var currentFrameDuration: TimeInterval {
    frames[currentFrameIndex]?.duration ?? .infinity
  }
  ...
  private func frame(at index: Int) -> UIImage? {
    ...
    let options: [CFString: Any]? = if resize {
      [
...
extension GifAnimator {
  struct GifFrame {
    ...
    var placeholderFrame: GifFrame {
      GifFrame(image: nil, duration: duration)
    }
  }
}
...
private class SafeArray<Element> {
  ...
  subscript(index: Int) -> Element? {
    ...
    set {
      lock.lock()
      defer { lock.unlock() }
      if let newValue, array.indices ~= index {
